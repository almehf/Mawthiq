...
export default function SettingsPage() {
  ...
  const loadSettings = async () => {
    try {
      const data = await base44.entities.OrganizationSettings.list('-created_date', 50);
      const settingsMap = {};
      data.forEach(s => {
  ...
  const handleSave = async () => {
    ...
    for (const entry of entries) {
      ...
      if (existing) {
        ...
        const allSettings = await base44.entities.OrganizationSettings.list('-created_date', 50);
        const record = allSettings.find(s => s.setting_key === entry.key);
        if (record) {
          await base44.entities.OrganizationSettings.update(record.id, {
