...
export default function Documents() {
  ...
  const loadData = async () => {
    ...
    try {
      const [docs, cols] = await Promise.all([
        ...
        base44.entities.Collection.filter({ status: 'active' }, '-created_date', 50),
      ]);
      setDocuments(docs);
      setCollections(cols);
    } finally {
  ...
  const handleApprove = async (doc) => {
    await base44.entities.Document.update(doc.id, { status: 'approved', reviewed_by_id: user.id, reviewed_by_name: user.full_name || user.email, reviewed_at: new Date().toISOString() });
    ...
    // Simulate indexing
    setTimeout(async () => {
      await base44.entities.Document.update(doc.id, { status: 'indexed', indexed_at: new Date().toISOString() });
      await base44.entities.AuditLog.create({
  ...
  const handleFlag = async (doc) => {
    await base44.entities.Document.update(doc.id, { status: 'flagged', reviewed_by_id: user.id, reviewed_by_name: user.full_name || user.email, reviewed_at: new Date().toISOString() });
    await base44.entities.AuditLog.create({
  ...
  const handleReject = async (doc) => {
    await base44.entities.Document.update(doc.id, { status: 'rejected', reviewed_by_id: user.id, reviewed_by_name: user.full_name || user.email, reviewed_at: new Date().toISOString() });
    await base44.entities.AuditLog.create({
